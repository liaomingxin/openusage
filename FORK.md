...
| `Sources/OpenUsage/Providers/ProviderCatalog.swift`、`ProviderAccountAssembly.swift` 的 `ownsDefaultHome` | **fork 独有行为（2026-09-17）**：上游规定「知道多个 Claude 账号后，日志里没写归属的会话一律不计」。但 Claude Code 2026-09 才开始写归属，且 bridge（远程驱动）会话写的是驱动端账号，于是本机大部分会话被丢掉、花费显示 No data。fork 给 `~/.claude` 那张卡加 `ownsDefaultHome: true`，`ProviderCatalog` 用 `card.allowsUnattributedPiUsage \|\| card.ownsDefaultHome` 兜底（日志扫描 + pi 都用它），即**默认卡按「本机设备」维度统计**：没有归属的会话，以及归属到「本机没有对应卡片的组织」的会话（bridge/远程会话就是这种），都算默认卡；`organizationsClaimedByOtherCards` 保证有自己卡片的组织不会被抢走，所以不会重复计数。Desktop 组织卡和 claude-swap 卡仍走严格规则。上游要是改这一段，保留 fork 的 OR 条件。另外 `ClaudeSessionIdentity.parse` 改成**按多数归属判定**（远程/bridge 会话会往日志里塞几行驱动端账号的归属记录，上游遇到两种归属就整段丢弃），只有真正各占一半才算 `conflicted`。回归测试：`ClaudeMachineLocalSpendTests`、`ClaudeSessionIdentityTests.testMajorityOwnerWinsOverStrayRecords` |
