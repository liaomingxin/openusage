...
The sections below sit in two columns, packed the same way the dashboard packs provider cards (each section joins whichever column is currently shorter), with a **Customize** row full width underneath. The sections are listed here in the order they're dealt out.
...
| Reset Times | Countdown / Exact time | "Resets in 3h 25m" vs "Resets today at 6:38 PM" — same toggle as clicking a reset label. It also sets how the subscription **Renews** row reads ("Renews in 20d 6h" vs "Renews Sep 16"; that row stops at the day, since a billing date has no meaningful time of day). |
