...
When any enabled provider tracks daily spend (Claude, Codex, Cursor, Grok, or OpenCode), a card sits above the provider sections. The title is a pull-down menu for **Cost**, **Cost/MTok**, or **Tokens** (Cost is the default; the choice sticks across restarts). A capsule switcher flips the period between **Today**, **Yesterday**, and **30 Days**. The ring sits on the left with the switcher and legend beside it; the legend keeps a readable width rather than stretching across the whole card. The ring, center total, and ranked legend follow the selected metric, and each legend row shows the provider's amount, its share of the total, and its token count (skipped in Tokens mode, where the amount already is tokens):
...
The ring center is always two short lines — a compact number on top and a quiet unit underneath (`$533` / `dollars`, `12.4` / `million`, or `$1.37` / `MTok`) — so Cost/MTok and big totals stay readable in the hole. Cost modes keep the `$` on the number. Hover the center for the exact one-line figure (and a note when any contributor's dollars are a local estimate — Cost and Cost/MTok only). Hovering a legend row or a ring segment highlights that provider: its arc grows slightly while the rest fade, and the center switches to that provider's amount and share until you move away. Each provider keeps a fixed color drawn from its brand (Claude's terracotta, OpenAI's green, and so on), and even a tiny share keeps a visible sliver of the ring. Providers with nothing for the selected metric simply don't appear — they're never counted as zero. (An enabled provider counts even if you've hidden its own spend rows in Customize; other dollar rows, like OpenRouter's API spend, never mix in.) The header's share icon (revealed when you hover the header, like a provider header's copy control) or right-clicking the card copies a branded PNG of the ring to your clipboard, just like sharing a provider card. The header also carries a small ⓘ naming the providers that feed the total. A period with nothing to show for the active metric shows a quiet empty state instead of hiding the card. Don't want the card at all? Turn it off with **Show Total Spend** at the top of [Settings](settings.md).

## Agent Usage

Beside the **Options** button in the dashboard footer, an **Agents** button (also in the Options menu) opens the **Agent Usage** screen: which local coding agents ran on this Mac, which models they used, and what that usage cost. The agents lay out as a two-column grid of cards in the dashboard's own anatomy — the agent's mark and name above a rounded card, with the agent's window cost and tokens on the header's trailing edge — covering Claude Code, Codex, Pi, OpenCode, Antigravity, Grok, Hermes, and Cursor. Inside a card, one ranked row per model: model name and cost on one line, share percentage and tokens on the next, with a thin share bar. Cards join whichever column is currently shorter, agents sort by spend (biggest spender first), and an agent with no usage in the window simply doesn't appear.

A capsule switcher at the top flips the period between **Today**, **Yesterday**, **7 Days**, and **30 Days** (the choice sticks across restarts), with the combined cost and tokens for that window beside it. All four windows come from one scan of the last 30 days, so switching is instant.

The figures come from each agent's own local logs (the same scans that back the spend rows, plus Hermes' local database; Cursor reads its refreshed usage export). Costs are what the agent itself reported where it reports one, otherwise an estimate from the bundled pricing snapshots. Models no pricing source can price are never folded into the totals — the agent card's amber warning names them instead. The screen refreshes on open when the last scan is more than one refresh interval old; the back button (or Esc) returns to the dashboard.
...
For Claude, Codex, Cursor, Grok, Kimi, OpenCode, and Z.ai spend rows, the value gently highlights when you point at it, signaling it's interactive; hovering it for a moment opens a small model breakdown for that period. When the source reported cache, each model also shows cache read, cache write, and cache hit rate (`cache read / (uncached input + cache write + cache read)`). A model that did not report cache leaves that line off rather than showing 0%. Z.ai adds a separate This Mac list that does not change the row total. Kimi's spend rows feed Total Spend. a ranked list of models, each showing its name and spend on one line, its share percentage and tokens on the next, and a thin share bar. Cursor groups its per-thinking-effort export slugs (like `claude-opus-4-8-thinking-max`) under the base model. Long tails fold into **Other** — anything past the top named models or under 5% of the period. Models no pricing source can price don't appear here (or in the row's totals) at all; the row's warning triangle names them instead (see [Pricing](pricing.md)).
...
**Usage Trend** (Claude, Codex, Cursor, Grok, and OpenCode) is a small bar chart of the last 30 days of token usage — one bar per day, drawn from the same source as that provider's spend rows (local logs for Claude, Codex, Grok, and OpenCode; Cursor's usage export for Cursor). **Hover it** for the peak day, the date range, your **Current Streak** and **Longest Streak** of consecutive days with any usage, and the source. A day with no usage breaks a streak; today being still empty doesn't — the current streak keeps counting through yesterday until today has usage or the day after begins. Both counts cover the chart's window, so a streak that fills it reads `31+ Days`; see [Usage history](usage-history.md) for why older days don't exist to count. It's on by default; turn it off or reorder it from Customize like any other metric. It can't be starred for the menu bar — the strip shows single values, not a chart.
...
The **provider list** shows every provider with a switch to turn it on or off, a count of its metrics, and a chevron into its detail. The rows sit in two cards side by side, dealt out in reading order (first row left, second right, and so on), so the list stays short on the wide popover. Turn a provider off and it stays in the list, greyed — its metrics hide from the dashboard and menu bar but keep their setup for when you turn it back on. Drag enabled providers by their grip to reorder — across both cards; dropping onto a row takes that row's place and the rest re-deal; tap a row to open its detail. On a fresh install only the providers detected on your Mac start on (see "First launch" above); this list is where you add the rest. A **Settings** row sits full width under the two cards.

A provider's **detail** has a back button and provider-specific Reset control in its top bar, followed by two metric cards side by side: **Always Visible** (shown on the dashboard card) on the left and **On Demand** (tucked behind the card's caret) on the right. Each metric row has a drag grip, its name, an always-visible star for the menu bar, and an on/off switch. Drag a metric into the other card—or onto one of that card's rows—to move it between Always Visible and On Demand. An empty card shows a dashed **Drag metrics here** target. You can star up to two metrics per provider. OpenRouter and Z.ai also show an **API Key** section here, where you can add, replace, reveal, or clear that provider's key. Z.ai's card also carries a **Platform** picker (Global `api.z.ai` or China `open.bigmodel.cn`), since a Z.ai key only works on the console that issued it — see [Z.ai](providers/zai.md).
